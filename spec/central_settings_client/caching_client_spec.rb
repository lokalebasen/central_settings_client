require 'spec_helper'
require_relative '../../lib/central_settings_client/caching_client'

describe CentralSettingsClient::CachingClient do
  let(:site_key) { 'dk' }
  let(:client) do
  ...
  describe 'checking server health' do
    it 'returns true if the status is 200' do
      allow_any_instance_of(CentralSettingsClient::Client)
  ...
  describe 'by site key' do
    it 'returns the value from the cache' do
      allow_any_instance_of(CentralSettingsClient::RobustSettingsCache)
        .to receive(:cached)
